...
export class ParticipantListComponent extends SidebarMixin implements OnInit {
  ...
  // ==================== DÉTAIL DES FORMATIONS DU PARTICIPANT ====================
  openDetail(participant: any): void {
    ...
    // Charger toutes les formations et filtrer celles qui contiennent ce participant
  ...
  submitAdd(): void {
    ...
    const participantData = {
      ...
      tel: formValue.tel,
      structure: { id: formValue.structure.id },
      profil: { id: formValue.profil.id }
    };

    console.log('Envoi participant:', participantData);

    this.http.post<any>(this.apiUrl, participantData).subscribe({
      next: (participant) => {
